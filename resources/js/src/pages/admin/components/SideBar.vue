    ...
    <div class="min:h-screen flex">
        <nav :class="`h-full bg-slate-50 ${toggleSidebar ? 'w-[210px]' : ''} `">
            <div class="flex flex-row justify-between p-3">
                <img
                    v-show="toggleSidebar ? true : false"
                    :src="App.baseUrl + '/images/logo.png'"
                    class="w-10"
                    src=""
                    alt=""
                />
                <button
                    @click="toggle"
                    class="hover:bg-slate-200 cursor-pointer p-2 rounded-md"
                >
            ...
            <ul class="flex flex-col gap-1 p-3">
                <li
                    class="flex gap-1 bg-slate-200 cursor-pointer p-2 rounded-md"
                >
                    <router-link to="/welcome" class="flex gap-1">
                        <HomeIcon class="mt-1" />
                        <span v-show="toggleSidebar">Home</span>
                    </router-link>
                </li>
                <li
                    v-show="users?.user?.role == ADMIN_ROLE"
                    class="flex gap-1 hover:bg-slate-200 cursor-pointer p-2 rounded-md"
                >
                ...
                <hr />
                <li
                    class="flex gap-1 text-red-600 hover:bg-slate-200 cursor-pointer p-2 rounded-md"
                >
        ...
        <div class="bg-slate-200 w-full">
            <div class="flex justify-between">
                ...
                <div class="p-3">
                    <img
                        @click="toggleTopManuBar"
                        :src="App.baseUrl + '/images/user.png'"
                        class="rounded-full w-10 h-10 cursor-pointer border-2 hover:border-indigo-600"
                        alt="logo"
                    />
                    <ul
                        v-show="toggleTopManu"
