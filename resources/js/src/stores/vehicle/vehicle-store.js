...
export const useVehicleStore = defineStore('vehicle-store', () => {
    ...
    async function getVehicles(page = 1){
        try{
            loading.value = true
            const data = await getData(`/vehicles?page=${page}&query=${query.value}`);
            // console.log(data);
            vehicles.value = data;
            loading.value = false
        }catch(errors){
    ...
    return { vehicles,loading,getVehicles,query,page,modalVal,toggleModal,deleteVehicle,vehicleId,vehicleInput,vVehicle$,addOrUpdateVehicle,edit}
})
if(import.meta.hot) {
